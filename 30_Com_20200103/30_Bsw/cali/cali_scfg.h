...
{
	ADC_vReadRaw(ADC_CFG_E_INDEX_PA1_AIN1_IntVsb, pData);
	*pData = (uint16)(((uint32)*pData * RTE_U16_10mV_V_VSB_INT_SCALE_FACT)>>12);
}
SINLINE void CALI_SCFG_vGetVsbExtVolt(uint16 *pData)
{
	ADC_vReadRaw(ADC_CFG_E_INDEX_PA5_AIN5_ExtVsb, pData);
	*pData = (uint16)(((uint32)*pData * RTE_U16_10mV_V_VSB_EXT_SCALE_FACT)>>12);
}
