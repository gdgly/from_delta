...
#    print(unF, unQ, start_time, end_time, unFflag, unQflag)
    for k in range(0, N):
        ...
        if 150<=k and k<=300:
            x = unF * x + np.sqrt(np.mat([[0.01, 0.1], [0.1, 1]])) * np.random.randn(nx, 1)
...
#    Nmin=np.mat(ast.literal_eval(str_Nmin.get()))
#    Nmax=np.mat(ast.literal_eval(str_Nmax.get()))
    NoptArr=arange(3,7+1)
...
#    figure_nopt.clf()
    ...
    fignopt.plot(NoptArr[1:-1],b[0,1:-1],label='Derivate of V(N)')
...
def rmse(systemModel,filterModel,unsystemModel,nMonte,smoothingFlag):
    N=systemModel.N
    ...
    for i in range(0,nMonte):
        ...
        if chVarKal.get()==1:
            #Kalman filter
            returnK= Kalman(systemModel,filterModel,unsystemModel,smoothingFlag)
        ...
        if chVarUF.get()==1:
            #UFIR filter
            resultFF=UFIR(systemModel, filterModel,unsystemModel,False)
...
def draw(flag):#画图
    ...
    if flag==1:
        ...
        a.plot(xhatFerror[0,:],label='Ffilter',linewidth=0.5)
        ...
        b.plot(xhatFrmse[0,:],label='Ffilter',linewidth=0.5)
        b.plot(xhatKrmse[0,:],label='Kfilter',linewidth=0.5)
...
def main():
    if int(chVarKal.get())==1 and int(chVarUF.get())==1:
        ...
        messagebox.showwarning("Error", "Please choose filter method")
...
################参数设置#####################################
    ...
    Nopt_run.grid(column=0,columnspan=2,row=3,padx=10,sticky=W+E)
    
    str_NN = StringVar()
    entry_Nopt_N = Entry(Nopt_parameter_frame, textvariable =str_NN ,state='normal',width = 10)
    entry_Nopt_N.grid(column=1, columnspan=1, row=0, rowspan=1,sticky=W)
    str_Nmin = StringVar()
    entry_Nopt_Nmin = Entry(Nopt_parameter_frame, textvariable = str_Nmin,state='normal',width = 10)
    entry_Nopt_Nmin.grid(column=1, columnspan=1, row=1, rowspan=1,sticky=W)    
    str_Nmax = StringVar()
    entry_Nopt_Nmax = Entry(Nopt_parameter_frame, textvariable = str_Nmax,state='normal',width = 10)
    entry_Nopt_Nmax.grid(column=1, columnspan=1, row=2, rowspan=1,sticky=W)
    
    NoptValue=12
    label_NoptValue=Label(Nopt_parameter_frame,text=NoptValue,fg='blue',font=('Times',15,'bold'))
    label_NoptValue.grid(column=1, columnspan=1, row=4, rowspan=1,sticky=W)
...
def checkunF():
    ...
    if chVarnoise.get()==1:
        if chVarunF.get()==1:
            messagebox.showwarning("Error", "Choose one uncertain condition only")
            checkunF.deselect()
            entry_unF['state']='disabled'
...
choice_frame=LabelFrame(master,text='Method Configuration',fg='red',padx=0,pady=0)
...
p1.grid(row=3,column=0,sticky=E+W,padx=5,pady=0)
p1["maximum"] = 100
p1["value"] = 0
...
fig_frame=LabelFrame(master,text='Figure',fg='red',padx=0,pady=0)
...
dataPlot = FigureCanvasTkAgg(f, fig_frame)
#dataPlot.get_tk_widget().grid(column=0,columnspan=2,row=0)
dataPlot.get_tk_widget().pack()

v=StringVar()
radiobutton_error=Radiobutton(fig_frame,text='Estimation error',
                              variable=v,value=1,indicatoron=False,command=lambda:figerror())
#radiobutton_error.grid(column=0, row=1, rowspan =1, \
#                sticky=W,padx = 10)
radiobutton_error.select()
radiobutton_error.pack(side=LEFT)

radiobutton_rmse=Radiobutton(fig_frame,text='Root mean square error',
                              variable=v,value=2,indicatoron=False,command=lambda:figrmse())
#radiobutton_rmse.grid(column=1, row=1, rowspan =1, \
#                sticky=W,padx = 10)
