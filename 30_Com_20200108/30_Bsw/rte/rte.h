...
#define RTE_B_TEMP_INLET_OTW		 RTE_uTempOtwStatus.Bits.f4
...
#define RTE_B_TEMP_INLET_OTP		 RTE_uTempOtpStatus.Bits.f4
...
#define RTE_LOW_LINE_50HZ            (0u)         /* Low line 50Hz, AC present;*/ 
#define RTE_LOW_LINE_60HZ            (4u)         /* Low line 60Hz, AC present;*/ 
#define RTE_HIGH_LINE_50HZ           (2u)         /* HIGH line 50Hz, AC present;*/ 
#define RTE_HIGH_LINE_60HZ           (6u)         /* HIGH line 60Hz, AC present;*/ 
#define RTE_NO_AC_INPUT              (1u)         /* No AC input*/
#define RTE_RESERVED                 (7u)         /* Reserved*/
...
#define RTE_B_COM_VIN_OCW                     RTE_uComStatus01.Bits.fe
...
typedef union PMBUS_U_UPD_STATUS1_
{
  ...
  struct
  {
    ...
    uint16 MFR_POS_LAST:1;       /* bitA */
		uint16 TEMP_OUTLET_UPDTING : 1; /* bitB */
		uint16 TEMP_LLC_UPDTING : 1; /* bitC */
		uint16 TEMP_RECDIO_UPDTING : 1; /* bitD */
    uint16 RESERVED : 2;         /* bitE~F */
  } Bits;
} PMBUS_U_UPD_STATUS1;
...
typedef struct PMBUS_S_DATA_
{
  ...
  WORD_VAL u16Temperatue_4_Linear;
	WORD_VAL u16Temperatue_5_Linear;
...
FOR NOKIA sHUB 2KW
********************************/
typedef struct PSMI_S_SMB_MASK_
{
  uint16 u16PsmiCrrentReg;
  uint16 u16Psmi_5E;
  uint16 u16Psmi_5F;
  uint16 u16Psmi_60;
  uint16 u16Psmi_61;
...
typedef union PSMI_U_SHUTDOWN_EVENT
{
  uint16 ALL;
  ...
  struct
  {
    ...
    uint8 FAN1_FAIL : 1; /* bit4 */
    uint8 FAN2_FAIL : 1; /* bit5 */
  } Bits;
} PSMI_U_SHUTDOWN_EVENT;

typedef union PSMI_U_THERMAL_WARNING
{
  uint16 ALL;
  ...
  {
		uint8 RESERVE : 2;
    uint8 T3_WARNING : 1; /* bit2 */
    uint8 T4_WARNING : 1; /* bit3 */
    uint8 F1_WARNING : 1; /* bit4 */
    uint8 F2_WARNING : 1; /* bit5 */
  } Bits;
} PSMI_U_THERMAL_WARNING;

typedef union PSMI_U_CURRENT_WARNING
{
  uint16 ALL;
  ...
  struct
  {
    uint8 MAIN_OC : 1; /* bit0 */
    uint8 STBY_OC : 1; /* bit1 */
  } Bits;
} PSMI_U_CURRENT_WARNING;

typedef union PSMI_U_INPUT_WARNING
{
  uint16 ALL;
  ...
  struct
  {
    uint8 IIN_OCW : 1; /* bit0 */
		uint8 RESERVE : 2;
    uint8 VIN_UVW : 1; /* bit3 */
  } Bits;
} PSMI_U_INPUT_WARNING;

typedef union PSMI_U_STATUS
{
  uint16 ALL;
  ...
  struct
  {
    ...
    uint8 PWOK : 1; /* bit4 */
    uint8 PSON : 1; /* bit5 */
		uint8 INTERRUPT : 1; /* bit6 */
		uint8 POWER_SUPPLY_RANGE : 2; /* bit7-8 */
  } Bits;
} PSMI_U_STATUS;

typedef struct PSMI_S_STATUS
{
  PSMI_U_SHUTDOWN_EVENT    u16StatusShutdown;
  PSMI_U_THERMAL_WARNING    u16StatusThermal;
  PSMI_U_CURRENT_WARNING    u16StatusIout;
  PSMI_U_INPUT_WARNING    u16StatusInput;
  PSMI_U_STATUS   				u16Status;
} PSMI_S_STATUS;
...
extern WORD_VAL RTE_uFanDuty[2];
/***********************************************
 * I2C
 **********************************************/
 ...
 * PSMI
 **********************************************/
extern PSMI_S_STATUS  PSMI_tStatus;
extern PSMI_S_STATUS  PSMI_tStatusOld;
